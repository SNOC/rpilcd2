...
# Load fonts
smallFont = ImageFont.truetype('Xerox Serif Narrow.ttf', 14)
largeFont = ImageFont.truetype('Xerox Serif Narrow.ttf', 20)
bigFont = ImageFont.truetype('Xerox Serif Narrow.ttf', 28)
...
while True:
	if mode == 0: # display date and time
		...
		if (str1 + str2 + str3) != lastString:
			...
			draw.text(((disp.width - sizeX) / 2, (disp.height - sizeY) / 2), str2,  font=bigFont, fill=255)
			sizeX, sizeY = draw.textsize(str3, font=smallFont)
			draw.text((disp.width - sizeX - padding, disp.height - sizeY - padding), str3,  font=smallFont, fill=255)
			disp.image(image)
			disp.display()
		...
		if (str1 + str2) != lastString:
			...
			draw.text(((disp.width - size1X) / 2, (disp.height - (size1Y + size2Y + padding)) / 2), str1,  font=largeFont, fill=255)
			draw.text(((disp.width - size2X) / 2, (disp.height - (size1Y + size2Y + padding)) / 2 + padding + size1Y), str2,  font=largeFont, fill=255)
			disp.image(image)
			disp.display()
